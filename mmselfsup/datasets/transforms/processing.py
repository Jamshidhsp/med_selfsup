...
class ColorJitter(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = results['img'].astype('unint16')
        for fn_id in order:
...
@TRANSFORMS.register_module()
class LoadImageFromFile_v1(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        try:
            ...
            # mean=np.array(self.mean.tolist()*3).astype(np.float32)
            # std=np.array(self.std.tolist()*3).astype(np.float32)
...
@TRANSFORMS.register_module()
class Normalize_v1(BaseTransform):
    """Normalize the image.
    ...
    def __init__(self, mean, std, to_rgb=False, to_three_chans=False):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        self.to_rgb = to_rgb
        self.to_three_chans = to_three_chans
    ...
    def __call__(self, results):
        ...
        if self.to_three_chans:
            img = results["img"][..., None]
            results["img"] = np.concatenate((img, img, img), axis=2)
            mean = np.array(self.mean.tolist() * 3).astype(np.float32)
            std = np.array(self.std.tolist() * 3).astype(np.float32)
        else:
            mean = self.mean
